...
class ArcMarkDiagram:
    ...
    def to_svg(self) -> str:
        ...
        for relationship in self.relationships:
            ...
            edges.append(f'<path d="M{x1} {y1} C{x1 + 70} {y1}, {x2 - 70} {y2}, {x2} {y2}"/><text x="{(x1+x2)/2}" y="{(y1+y2)/2-9}">{escape(relationship.type)}</text>')
        cards = []
